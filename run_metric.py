...
# Silence SHAP / explain. SHAP made it so some logs print massive matrices on the INFO level. Who thought that was a good idea?
...
###############################################################################
#        KeyBERT_TFIDFPipelineRunner  – uses the cached attach_keywords       #
###############################################################################
...
def main():
    ...
    parser.add_argument("metric_table", help="Name of the metric table to process")
    args = parser.parse_args()

    metric_table_name = args.metric_table
    ...
    try:
        ...
        # ── results folder ─────────────────────────────────────────
        results_dir = RESULTS_DIR / f"results_{current_timestamp}" / metric_table_name
        results_dir.mkdir(parents=True, exist_ok=True)

        # ── run pipelines ──────────────────────────────────────────
        unsupervised_results = run_unsupervised_models(df.copy(), output_dir=results_dir)
